import Head from 'next/head'
import Image from 'next/image'
import {
	motion,
	useAnimation,
	useViewportScroll,
	useTransform,
	useSpring,
} from 'framer-motion'

export default function MendocinoSpirits() {
	return (
		<div>
			<Head>
				<title>Color Mill Design - Mendocino Spirits</title>
				<link rel='icon' href='/favicon.ico' />
			</Head>
			<main>
				<section className='px-[5vw] mt-24 h-[180vh] overflow-hidden'>
					<div className='w-screen ml-12'>
						<h2 className='font-alice text-6xl text-[#23160c]'>
							Mendocino Spirits
						</h2>
						<svg height='6' width='500' className='mt-6'>
							<line
								x1='0'
								y1='0'
								x2='500'
								y2='0'
								style={{ stroke: '#23160c', strokeWidth: 5 }}
							/>
						</svg>
					</div>
					<div className='relative mt-16'>
						<div className='absolute h-auto p-16 ml-12 top-40 left-0 w-1/2 bg-[#F5E4C5] z-20'>
							<h3>
								Lorem ipsum dolor sit amet, consectetur
								adipiscing elit, sed do eiusmod tempor
								incididunt ut labore et dolore magna aliqua. Ut
								enim ad minim veniam, quis nostrud exercitation
								ullamco laboris nisi ut aliquip ex ea commodo
								consequat. Duis aute irure dolor in
								reprehenderit in voluptate velit esse cillum
								dolore eu fugiat nulla pariatur. Excepteur sint
								occaecat cupidatat non proident, sunt in culpa
								qui officia deserunt mollit anim id est laborum.
							</h3>
						</div>
						<div className='absolute ml-[25vw] top-0 right-0 z-10'>
							<Image
								src='v1616447175/Color%20Mill%20Design/mendocino-spirits-cover_tzgrip.jpg'
								layout='intrinsic'
								width='1920'
								height='1280'
								alt='Mendocino Spirits'
								quality='100'></Image>
						</div>
						<div className='absolute top-[600px] bg-[#57a89b] h-[900px] w-screen -ml-24 z-0'>
							<Image
								src='v1618268630/Color%20Mill%20Design/Mendocino-Spirits-wallpaper_wsyhyj.svg'
								width='1440'
								height='900px'
								layout='responsive'></Image>
						</div>
						<div className='absolute h-auto p-16 ml-12 top-[700px] left-0 w-1/2 bg-[#F5E4C5] z-30'>
							<h4 className='font-alice text-5xl text-[#23160c]'>
								Alice
							</h4>
							<h5 className='font-alice text-3xl text-[#23160c] mb-12'>
								Aa Bb Cc Dd Ee Ff Gg Hh Ii Jj Kk Ll Mm Nn Oo Pp
								Qq Rr Ss Tt Uu Vv Ww Xx Yy Zz
							</h5>
							<h4 className='font-fira text-4xl text-[#23160c]'>
								Fira Sans
							</h4>
							<h5 className='font-fira text-2xl text-[#23160c]'>
								Aa Bb Cc Dd Ee Ff Gg Hh Ii Jj Kk Ll Mm Nn Oo Pp
								Qq Rr Ss Tt Uu Vv Ww Xx Yy Zz
							</h5>
							<ul className=' bg-white w-[1500px] ml-[150px] py-8 px-8 grid grid-cols-10 gap-12 mt-12'>
								<li>
									<div className=' h-24 w-24 bg-[#f5e4c5] text-lg'></div>
									<h5 className='mt=2 '>#f5e4c5</h5>
								</li>
								<li>
									<div className=' h-24 w-24 bg-[#552919] text-lg'></div>
									<h5 className='mt=2 '>#552919</h5>
								</li>
								<li>
									<div className=' h-24 w-24 bg-[#8e513a] text-lg'></div>
									<h5 className='mt=2 '>#8e513a</h5>
								</li>
								<li>
									<div className=' h-24 w-24 bg-[#bf8d53] text-lg'></div>
									<h5 className='mt=2 '>#bf8d53</h5>
								</li>
								<li>
									<div className=' h-24 w-24 bg-[#23160c] text-lg'></div>
									<h5 className='mt=2 '>#23160c</h5>
								</li>
								<li>
									<div className=' h-24 w-24 bg-[#f3c268] text-lg'></div>
									<h5 className='mt=2 '>#f3c268</h5>
								</li>
								<li>
									<div className=' h-24 w-24 bg-[#a6a36d] text-lg'></div>
									<h5 className='mt=2 '>#a6a36d</h5>
								</li>
								<li>
									<div className=' h-24 w-24 bg-[#57a89b] text-lg'></div>
									<h5 className='mt=2 '>#57a89b</h5>
								</li>
								<li>
									<div className=' h-24 w-24 bg-[#e68242] text-lg'></div>
									<h5 className='mt=2 '>#e68242</h5>
								</li>
								<li>
									<div className=' h-24 w-24 bg-[#077893] text-lg'></div>
									<h5 className='mt=2 '>#077893</h5>
								</li>
							</ul>
						</div>
					</div>
				</section>
				<section className='px-[5vw] mt-24  overflow-hidden'>
					<div className='w-screen ml-12'>
						<h2 className='font-alice text-6xl text-[#23160c]'>
							Design Brief
						</h2>
						<svg height='6' width='500' className='mt-6'>
							<line
								x1='0'
								y1='0'
								x2='500'
								y2='0'
								style={{ stroke: '#23160c', strokeWidth: 5 }}
							/>
						</svg>
						<h3 className='w-3/4 mt-12 text-lg'>
							Lorem ipsum dolor sit amet, consectetur adipiscing
							elit, sed do eiusmod tempor incididunt ut labore et
							dolore magna aliqua. Ut enim ad minim veniam, quis
							nostrud exercitation ullamco laboris nisi ut aliquip
							ex ea commodo consequat. Duis aute irure dolor in
							reprehenderit in voluptate velit esse cillum dolore
							eu fugiat nulla pariatur. Excepteur sint occaecat
							cupidatat non proident, sunt in culpa qui officia
							deserunt mollit anim id est laborum.
						</h3>
					</div>
				</section>
				<section className='px-[5vw] mt-24  overflow-hidden'>
					<div className='w-screen ml-12'>
						<h2 className='font-alice text-6xl text-[#23160c]'>
							Stylescape
						</h2>
						<svg height='6' width='500' className='mt-6 mb-12'>
							<line
								x1='0'
								y1='0'
								x2='500'
								y2='0'
								style={{ stroke: '#23160c', strokeWidth: 5 }}
							/>
						</svg>
						<Image
							src='v1618251531/Color%20Mill%20Design/Mendocino-Spirits-Stylescape_md0ylw.jpg'
							layout='intrinsic'
							width='1920'
							height='420'
							alt='Mendocino Spirits'
							quality='100'></Image>
					</div>
				</section>
				<section className='mx-[5vw] mt-24 '>
					<div className=' ml-12'>
						<h2 className='font-alice text-6xl text-[#23160c]'>
							Bottle Labels
						</h2>
						<svg height='6' width='500' className='mt-6 mb-12'>
							<line
								x1='0'
								y1='0'
								x2='500'
								y2='0'
								style={{ stroke: '#23160c', strokeWidth: 5 }}
							/>
						</svg>
					</div>
					<div className='grid grid-rows-2 grid-flow-col gap-24 w-3/4 mx-auto'>
						<div className='row-span-2 '>
							<Image
								src='v1618350804/Color%20Mill%20Design/Mendocino-Spirits-3-labels_rrdnxk.jpg'
								layout='intrinsic'
								height='1440px'
								width='960px'
								quality='80'
							/>
						</div>
						<div className='row-span-1'>
							<Image
								src='v1618329878/Color%20Mill%20Design/Mendocino-Spirits-Aged-Gin-tree_qb2bhj.jpg'
								layout='intrinsic'
								height='720px'
								width='1080px'
								quality='80'
							/>
						</div>
						<div className='row-span-1'>
							<Image
								src='v1618329504/Color%20Mill%20Design/Mendocino-Spirits-Captain-Fletcher-Lighthouse_gwesfb.jpg'
								layout='intrinsic'
								height='720px'
								width='1080px'
								quality='80'
							/>
						</div>
					</div>
					<div className='grid grid-rows-1 grid-flow-col gap-48 mt-12 w-3/4 m-12 mx-auto'>
						<Image
							src='v1618251040/Color%20Mill%20Design/Mendocino-Spirits-Bourbon-label_boulyi.jpg'
							width='414'
							height='1089'
							layout='intrinsic'
							quality='80'
						/>
						<Image
							src='v1618251041/Color%20Mill%20Design/Mendocino-Spirits-CaptainFletcher-label-01_qhcxr5.jpg'
							width='414'
							height='1089'
							layout='intrinsic'
							quality='80'
						/>
						<Image
							src='v1618251043/Color%20Mill%20Design/Mendocnio-Spirits-AgedGin-Label_wzgdrp.jpg'
							width='414'
							height='1089'
							layout='intrinsic'
							quality='80'
						/>
					</div>
				</section>
				<section className='mx-[5vw] mt-24 '>
					<div className=' ml-12'>
						<h2 className='font-alice text-6xl text-[#23160c]'>
							Printed Collateral
						</h2>
						<svg height='6' width='500' className='mt-6 mb-12'>
							<line
								x1='0'
								y1='0'
								x2='500'
								y2='0'
								style={{ stroke: '#23160c', strokeWidth: 5 }}
							/>
						</svg>
					</div>
					<div className='mx-auto w-7/12'>
						<Image
							src='v1618329431/Color%20Mill%20Design/Mendocino-Spirits-Brochure-and-Cards_cvrp69.jpg'
							layout='intrinsic'
							width='1440'
							height='960'
							alt='Mendocino Spirits'
							quality='80'></Image>
					</div>
					<div className='grid grid-cols-2 grid-rows-3 grid-flow-col gap-8 w-10/12 mx-auto mt-12'>
						<div className='col-span-1 row-span-2'>
							<Image
								src='v1618360236/Color%20Mill%20Design/Mendocino-Spirits-Brochure-01_es8fhg.jpg'
								layout='intrinsic'
								height='793'
								width='1020'
								quality='80'
							/>
						</div>
						<div className='col-span-1 row-start-2 row-span-2'>
							<Image
								src='v1618360236/Color%20Mill%20Design/Mendocino-Spirits-Brochure-02_ncnvdk.jpg'
								layout='intrinsic'
								height='793'
								width='1020'
								quality='80'
							/>
						</div>
					</div>
				</section>
			</main>
		</div>
	)
}
